...
package net.mcreator.testestsetest.keybind;
...
import net.mcreator.testestsetest.procedures.YrdOnKeyPressedProcedure;
...
public class YrdKeyBinding extends TestestsetestModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public YrdKeyBinding(TestestsetestModElements instance) {
		super(instance, 102);
	...
	public void initElements() {
		keys = new KeyBinding("key.testestsetest.yrd", GLFW.GLFW_KEY_Z, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					TestestsetestMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				YrdOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
